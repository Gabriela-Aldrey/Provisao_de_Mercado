...
def get_selic_live():
    ...
    try:
        url = "https://api.bcb.gov.br/dados/serie/bcdata.sgs.432/dados?formato=json&dataInicial=01/07/2016"
        response = requests.get(url, timeout=10)
        data = response.json()
        df = pd.DataFrame(data)
        df['date'] = pd.to_datetime(df['data'], format='%d/%m/%Y', dayfirst=True)
        ...
        return pd.DataFrame({'date': dates, 'selic_rate': [14.25]*60})
...
with st.expander("📈 **Histórico Completo SELIC (2016-2026) - Dados Oficiais BCB**", expanded=True):
    ...
    if selic_complete is not None and selic_2020 is not None:
        ...
        st.markdown("---")
        st.markdown("## **ANÁLISE SELIC (2020 - Jul/2026)**")
